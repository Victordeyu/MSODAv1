...
conf=-0.25
u_conf=-0.6
make_print_to_file(filename='{}_Mlti_2{}_v1_2'.format(dataSet,tg),path='logs')
print("DataSet:{}".format(root),"\nTarget Domain:{}".format(tg))

Xs,ys,Xt,yt,l,Cs,Cu=readDataMS(root,scs,tg,Cs_end,Cu_start,fn='fts',postfix='_Al7')
...
# l=np.hstack((np.array(l[:index[-1]]),np.full((Xt_new.shape[0],),domain_num-1)))
...
max_known_expected=round(len(Xt_new)/15)
max_unknown_expected=round(len(Xt_new)*22/33)
print(max_known_expected,max_unknown_expected)


for i in range(8):
    W = cost_manifold(Ws, Wt,l, Cs, Xs_all, ys_all, Xt_new, Yt_pseudo, 10, 100).point
    ...
    if i<9:
        class_remain=np.zeros(Cs+1)
    known_expected=0
    unknown_expected=0
    if i<5:
        known_expected=round(max_known_expected*(i+1)/5)
        unknown_expected=round(max_unknown_expected*(i+1)/5)
    else:
        known_expected=max_known_expected
        unknown_expected=max_unknown_expected
    for j in range(0,Cs):
        class_remain[j]=known_expected
    class_remain[Cs]=unknown_expected
    
    
    predict_Xt = predict[:, Xs_pre.shape[0]:]  # 分类模型对目标域样本的置信度矩阵
    predict_Xt = predict_Xt / np.sum(predict_Xt, axis=0)[None, :]  # 置信度归1
    print(predict_Xt.shape)
    if i<9:
        weight=predict_Xt.max(axis=0)
    Yt_pseudo = predict_Xt.argmax(axis=0)#Yt_pseudo是模型的预测输出类，也是伪标签
    if i<9:
        index=np.argsort(weight)
        for j in range(0,len(weight)):
            if class_remain[Yt_pseudo[index[len(weight)-1-j]]] > 0:
                class_remain[Yt_pseudo[index[len(weight)-1-j]]]-=1
            else:
                Yt_pseudo[index[len(weight)-1-j]]=Cs+1
