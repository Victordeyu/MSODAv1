...
def OSNNcv(Xs,Ys,Xt,Yt,T):
    ...
    c=len(np.unique(Ys))
    Cls=np.zeros((Xt.shape[0],))
    Dist=cdist(Xs,Xt)
    nt,mt=Xt.shape
    ns,ms=Xs.shape
    Min_index=np.argsort(Dist,axis=0)[:2,:]
    # print(Min_index.shape)
    for i in range(nt):
        Min_label1,Min_label2=Ys[Min_index[0,i]],Ys[Min_index[1,i]]
        Dist1,Dist2=Dist[0,i],Dist[1,i]
        # print(Dist1,Dist2)
        if Min_label1==Min_label2:
            Cls[i]=Min_label1
        elif Dist1<=T*Dist2:
            Cls[i]=Min_label1
        elif Dist2<=Dist1:
...
def make_print_to_file(filename="Default.log",path='./'):
    ...
    fileName = filename+datetime.datetime.now().strftime('day'+'%Y_%m_%d')
...
make_print_to_file(filename='{}_Mlti_2{}__'.format(dataSet,tg),path='logs')
...
# Yt_pseudo2,acc=OSNNcv(Xs,ys,Xt_new,Yt_new,0.9)
# print('OSNN pre',acc)
...
for i in range(40):
    ...
    loss, predict = model_loss(Xs_pre, ys_all , Xt_pre, Yt_pseudo, Cs, sigma, eta=0.001, gamma_tk=0.2, gamma_tu=0.7, gamma_s=0.7)
