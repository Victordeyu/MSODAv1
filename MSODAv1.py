...
#     acc_known = 0
#     for c in range(Cs):  # 计算已知类的分类准确率
#         known_i = np.sum(Ytk_pseudo[Ytk == c] == Ytk[Ytk == c])
#         acc_known = acc_known + known_i

#     acc_known = acc_known /Ytk.shape[0]
...
for i in range(10):
    ...
    loss, predict = model_loss(Xs_pre, ys_all , Xt_pre, Yt_pseudo, l,Cs, sigma, eta=eta, gamma_tk=gamma_tk, gamma_tu=gamma_tu, gamma_s=gamma_s)
...
for i in range(len(accs)):
    print("Iterations:",i,'\tacc{}\tacc_known{}\tacc_unknown{}'.format(accs[i],acc_knowns[i],acc_unknowns[i]))