...
scs=['imageclef-p.csv','imageclef-c.csv']
tg='imageclef-i.csv'
# domain_num=2
...
def Train(tg,scs,eta=0.002,gamma_tk=0.4,gamma_tu=0.5,gamma_s=0.57,conf=0):
    Cs_end,Cu_start=5,6
    make_print_to_file(filename='{}_Mlti_2{}_v1_2'.format(dataSet,tg),path='ImageCleflogs')
...
for tg in domain:
    ...
    print(tg,scs)
    Train(tg,scs)
# Eta=[i/1000 for i in range(1,10,1)]#eta
# Gamma_s=[i/100 for i in range(30,62,3)]#Gamma_s
# Gamma_tk=[i/100 for i in range(10,40,3)]
# Gamma_tu=[i/100 for i in range(30,55,3)]
# para=Gamma_s
# max_ACCs=[]
# for p in para:
#     max_acc=Train(gamma_tk=p)
#     max_ACCs.append(max_acc)
    
# for i,max_acc in enumerate(max_ACCs):
#     print("Gamma_s:{}\t Max_Acc:{}".format(para[i],max_acc))